...
```sql
SELECT COUNT(*) FROM state_groups WHERE room_id NOT IN (SELECT room_id FROM rooms);
SELECT COUNT(*) FROM state_groups_state WHERE room_id NOT IN (SELECT room_id FROM rooms);
SELECT COUNT(*) FROM device_lists_changes_in_room WHERE room_id NOT IN (SELECT room_id FROM rooms);
```
...
```sql
DELETE FROM state_groups WHERE room_id NOT IN (SELECT room_id FROM rooms);
DELETE FROM state_groups_state WHERE room_id NOT IN (SELECT room_id FROM rooms);
DELETE FROM device_lists_changes_in_room WHERE room_id NOT IN (SELECT room_id FROM rooms);
```
